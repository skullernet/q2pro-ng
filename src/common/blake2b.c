...
static void blake2b_init_param(blake2b_state *S, const blake2b_param *P)
{
    ...
    /* IV XOR ParamBlock */
    for (i = 0; i < 8; ++i)
        S->h[i] = blake2b_IV[i] ^ Q_RL64(p + sizeof(S->h[i]) * i);
...
void blake2b_final(blake2b_state *S, void *out, size_t outlen)
{
    ...
    Q_assert(out);
    Q_assert(outlen >= S->outlen);
    Q_assert(!blake2b_is_lastblock(S));
    ...
    for (i = 0; i < 8; i++)     /* Byte swap in place */
        S->h[i] = LittleLong64(S->h[i]);
