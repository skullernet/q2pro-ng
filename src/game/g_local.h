...
extern const box3_t player_box;
...
typedef enum : uint64_t {
    AI_NONE                 = 0ULL,
    AI_STAND_GROUND         = BIT_ULL(0),
    AI_TEMP_STAND_GROUND    = BIT_ULL(1),
    AI_SOUND_TARGET         = BIT_ULL(2),
    AI_LOST_SIGHT           = BIT_ULL(3),
    AI_PURSUIT_LAST_SEEN    = BIT_ULL(4),
    AI_PURSUE_NEXT          = BIT_ULL(5),
    AI_PURSUE_TEMP          = BIT_ULL(6),
    AI_HOLD_FRAME           = BIT_ULL(7),
    AI_GOOD_GUY             = BIT_ULL(8),
    AI_BRUTAL               = BIT_ULL(9),
    AI_NOSTEP               = BIT_ULL(10),
    AI_DUCKED               = BIT_ULL(11),
    AI_COMBAT_POINT         = BIT_ULL(12),
    AI_MEDIC                = BIT_ULL(13),
    AI_RESURRECTING         = BIT_ULL(14),

// ROGUE
    AI_MANUAL_STEERING      = BIT_ULL(15),
    AI_TARGET_ANGER         = BIT_ULL(16),
    AI_DODGING              = BIT_ULL(17),
    AI_CHARGING             = BIT_ULL(18),
    AI_HINT_PATH            = BIT_ULL(19),
    AI_IGNORE_SHOTS         = BIT_ULL(20),
// PMM - FIXME - last second added for E3 .. there's probably a better way to do this but
// this works
    AI_DO_NOT_COUNT         = BIT_ULL(21), // set for healed monsters
    AI_SPAWNED_COMMANDER    = BIT_ULL(22), // both do_not_count and spawned are set for spawned monsters
    AI_SPAWNED_NEEDS_GIB    = BIT_ULL(23), // only return commander slots when gibbed
    AI_RESERVED_24          = BIT_ULL(24),
    AI_BLOCKED              = BIT_ULL(25), // used by blocked_checkattack: set to say I'm attacking while blocked
                                            // (prevents run-attacks),
// ROGUE

    AI_SPAWNED_ALIVE        = BIT_ULL(26), // [Paril-KEX] for spawning dead
    AI_SPAWNED_DEAD         = BIT_ULL(27),
    AI_HIGH_TICK_RATE       = BIT_ULL(28), // not limited by 10hz actions
    AI_NO_PATH_FINDING      = BIT_ULL(29), // don't try nav nodes for path finding
    AI_PATHING              = BIT_ULL(30), // using nav nodes currently
    AI_STINKY               = BIT_ULL(31), // spawn flies
...
// flags saved when monster is respawned
    AI_RESPAWN_MASK         = AI_STINKY | AI_SPAWNED_COMMANDER | AI_SPAWNED_NEEDS_GIB,
// flags saved when a monster dies
    AI_DEATH_MASK           = AI_DOUBLE_TROUBLE | AI_GOOD_GUY | AI_RESPAWN_MASK,
} monster_ai_flags_t;
...
typedef enum : uint64_t {
    FL_NONE                 = 0ULL,
    FL_FLY                  = BIT_ULL(0),
    FL_SWIM                 = BIT_ULL(1),  // implied immunity to drowning
    FL_IMMUNE_LASER         = BIT_ULL(2),
    FL_INWATER              = BIT_ULL(3),
    FL_GODMODE              = BIT_ULL(4),
    FL_NOTARGET             = BIT_ULL(5),
    FL_IMMUNE_SLIME         = BIT_ULL(6),
    FL_IMMUNE_LAVA          = BIT_ULL(7),
    FL_PARTIALGROUND        = BIT_ULL(8),  // not all corners are valid
    FL_WATERJUMP            = BIT_ULL(9),  // player jumping out of water
    FL_TEAMSLAVE            = BIT_ULL(10), // not the first on the team
    FL_NO_KNOCKBACK         = BIT_ULL(11),
    FL_POWER_ARMOR          = BIT_ULL(12), // power armor (if any) is active

    // ROGUE
    FL_MECHANICAL           = BIT_ULL(13), // entity is mechanical, use sparks not blood
    FL_SAM_RAIMI            = BIT_ULL(14), // entity is in sam raimi cam mode
    FL_DISGUISED            = BIT_ULL(15), // entity is in disguise, monsters will not recognize.
    FL_NOGIB                = BIT_ULL(16), // player has been vaporized by a nuke, drop no gibs
    FL_DAMAGEABLE           = BIT_ULL(17),
    FL_STATIONARY           = BIT_ULL(18),
    // ROGUE

    FL_ALIVE_KNOCKBACK_ONLY = BIT_ULL(19), // only apply knockback if alive or on same frame as death
    FL_NO_DAMAGE_EFFECTS    = BIT_ULL(20),

    FL_COOP_HEALTH_SCALE    = BIT_ULL(21), // [Paril-KEX] gets scaled by coop health scaling
    FL_FLASHLIGHT           = BIT_ULL(22), // enable flashlight
    FL_KILL_VELOCITY        = BIT_ULL(23), // for berserker slam
    FL_NOVISIBLE            = BIT_ULL(24), // super invisibility
    FL_DODGE                = BIT_ULL(25), // monster should try to dodge this
    FL_TEAMMASTER           = BIT_ULL(26), // is a team master (only here so that entities abusing teammaster/teamchain for stuff don't break)
    FL_ALWAYS_TOUCH         = BIT_ULL(27), // always touch, even if we normally wouldn't
    FL_NO_STANDING          = BIT_ULL(28), // don't allow "standing" on non-brush entities
    FL_WANTS_POWER_ARMOR    = BIT_ULL(29), // for players, auto-shield

    FL_RESPAWN              = BIT_ULL(30), // used for item respawning
    FL_IMMORTAL             = BIT_ULL(31), // never go below 1hp

    FL_DEEPONE              = BIT_ULL(32), // never drown
    FL_ACIDIC               = BIT_ULL(33), // green blood

    // player entity flags that survive level change
    FL_CLIENT_SAVED_MASK    = FL_FLY | FL_GODMODE | FL_NOTARGET | FL_POWER_ARMOR | FL_FLASHLIGHT |
...
{
    return Q_lerpf(min_inclusive, max_exclusive, frandom());
}
...
bool M_droptofloor_generic(vec3_t *origin, box3_t box, bool ceiling, int ignore,
                           contents_t mask, bool allow_partial);
...
void M_CategorizePosition(edict_t *self, vec3_t in_point, water_level_t *waterlevel, contents_t *watertype);
...
// RAFAEL
void monster_fire_ionripper(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed,
                            monster_muzzleflash_id_t flashtype, effects_t effect);
...
void monster_fire_blueblaster(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed,
                              monster_muzzleflash_id_t flashtype, effects_t effect);
void G_Monster_CheckCoopHealthScaling(void);
// RAFAEL
// ROGUE
void monster_fire_blaster2(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed,
                           monster_muzzleflash_id_t flashtype, effects_t effect);
void monster_fire_tracker(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, edict_t *enemy,
                          monster_muzzleflash_id_t flashtype);
void monster_fire_heatbeam(edict_t *self, vec3_t start, vec3_t dir, vec3_t offset, int damage,
...
void gib_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, mod_t mod);
...
void fire_shotgun(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick,
...
edict_t *fire_blaster(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed,
                      effects_t effect, mod_t mod);
void Grenade_Explode(edict_t *ent);
void fire_grenade(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, gtime_t timer,
                  float damage_radius, float right_adjust, float up_adjust);
void fire_grenade2(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, gtime_t timer,
                   float damage_radius, bool held);
void rocket_touch(edict_t *ent, edict_t *other, const trace_t *tr, bool other_touching_self);
edict_t *fire_rocket(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed,
...
// RAFAEL
void fire_ionripper(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, effects_t effect);
void fire_heat(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius,
               int radius_damage, float turn_fraction);
void fire_blueblaster(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, effects_t effect);
...
// g_player.c
//
void player_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, mod_t mod);
...
bool M_CheckBottom_Slow_Generic(vec3_t origin, box3_t box, int ignore,
                                contents_t mask, bool ceiling, bool allow_any_step_height);
...
void fire_blaster2(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, effects_t effect, bool hyper);
...
bool M_CalculatePitchToFire(edict_t *self, vec3_t target, vec3_t start, vec3_t *aim, float speed, float time_remaining, bool mortar, bool destroy_on_touch);
...
edict_t *CreateFlyMonster(vec3_t origin, vec3_t angles, box3_t box,
                          const char *classname);
edict_t *CreateGroundMonster(vec3_t origin, vec3_t angles, box3_t box,
                             const char *classname, float height);
bool     FindSpawnPoint(vec3_t startpoint, box3_t box, vec3_t *spawnpoint,
...
struct edict_s {
    ...
    void (*die)(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, mod_t mod);
