...
int trap_FS_ReadFile(void *buffer, unsigned len, qhandle_t f);
int trap_FS_WriteFile(const void *buffer, unsigned len, qhandle_t f);
...
int trap_FS_ReadLine(qhandle_t f, char *buffer, unsigned size);
unsigned trap_FS_ListFiles(const char *path, const char *filter, unsigned flags, char *buffer, unsigned size);
unsigned trap_FS_ErrorString(int error, char *buf, unsigned size);
...
#define trap_ClientLayout gi.ClientLayout
#define trap_ClientStuffText gi.ClientStuffText
#define trap_ClientInventory gi.ClientInventory
