...
static void soldier_cock(edict_t *self)
{
    if (self->s.frame == FRAME_stand322)
        G_StartSound(self, CHAN_WEAPON, sound_cock, 1, ATTN_IDLE);
    else
        G_StartSound(self, CHAN_WEAPON, sound_cock, 1, ATTN_NORM);
...
static void soldierh_hyper_sound_end(edict_t *self)
{
    if (self->monsterinfo.weapon_sound) {
        if (soldier_style() == BlueHyper)
            G_StartSound(self, CHAN_AUTO, gi.soundindex("weapons/hyprbd1a.wav"), 1, ATTN_NORM);
...
{
    if (brandom())
        G_StartSound(self, CHAN_VOICE, sound_sight1, 1, ATTN_NORM);
    else
        G_StartSound(self, CHAN_VOICE, sound_sight2, 1, ATTN_NORM);
...
void DIE(soldier_die)(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, const vec3_t point, mod_t mod)
{
    ...
    if (is_light())
        G_StartSound(self, CHAN_VOICE, sound_death_light, 1, ATTN_NORM);
    else if (is_blitz())
